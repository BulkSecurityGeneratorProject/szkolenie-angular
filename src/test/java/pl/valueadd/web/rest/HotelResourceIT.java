...
public class HotelResourceIT {
    ...
    public void getAllHotelsWithEagerRelationshipsIsEnabled() throws Exception {
        HotelResource hotelResource = new HotelResource(hotelRepositoryMock);
        when(hotelRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        MockMvc restHotelMockMvc = MockMvcBuilders.standaloneSetup(hotelResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
            .setControllerAdvice(exceptionTranslator)
            .setConversionService(createFormattingConversionService())
            .setMessageConverters(jacksonMessageConverter).build();

        restHotelMockMvc.perform(get("/api/hotels?eagerload=true"))
        .andExpect(status().isOk());

        verify(hotelRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({"unchecked"})
    public void getAllHotelsWithEagerRelationshipsIsNotEnabled() throws Exception {
        HotelResource hotelResource = new HotelResource(hotelRepositoryMock);
            when(hotelRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
            MockMvc restHotelMockMvc = MockMvcBuilders.standaloneSetup(hotelResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
